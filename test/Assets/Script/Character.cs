...
public class Character : MonoBehaviour
{
    ...
    public void Throw()
    {
        ...
        for (int i = 0; i < 360; i += 15)
        {
            ...
            //TEST
            Collider2D myCollider = item.GetComponent<Collider2D>();
            ...
            angle1 = Math.Cos(Mathf.Deg2Rad * (angle + i));
            angle2 = Math.Sin(Mathf.Deg2Rad * (angle + i));
            double pointx = -1*radius * angle1 + this.transform.position.x;
            double pointy = -1*radius * angle2 + this.transform.position.y;
            Vector2 direction = new Vector2((float)pointx, (float)pointy) - new Vector2(this.transform.position.x, this.transform.position.y);
           // direction.x = dX != 0 ? direction.x * dX : direction.x*1;
            //direction.y *= dY != 0 ? direction.y * dY : direction.y*1 ;
            ...
            item.GetComponent<Rigidbody2D>().velocity = direction.normalized * this.ThrowSpeed;
